...
def save_output_masks(args, predictor):
    img_num = len(os.listdir(args.val_data_path))
    sum_IoU = 0
    output_mask_save_folder = os.path.join(args.output_mask_save_folder, "threshold_{}".format(args.scorce_threshold))
    if not os.path.exists(output_mask_save_folder):
        os.makedirs(output_mask_save_folder)
    for img_name in os.listdir(args.val_data_path):
        img_path = os.path.join(args.val_data_path, img_name)
        # print(img_path)
        im = cv2.imread(img_path)
        outputs = predictor(im)
        mask_exists = outputs["instances"].to("cpu").pred_masks.shape[0]
        mask_save_path = os.path.join(output_mask_save_folder, img_name.replace(".jpg",".png"))
        if mask_exists:
            mask_path = os.path.join(args.mask_folder, img_name.replace(".jpg",".png"))
            pred_mask, GT_mask = get_pred_and_GT_mask(mask_path,args.scorce_threshold, outputs["instances"].to("cpu").scores.numpy() \
                ,outputs["instances"].to("cpu").pred_masks.numpy())
            
            cv2.imwrite(mask_save_path, pred_mask.astype(int)*255)
            # plt.imsave(mask_save_path, pred_mask.astype(int), cmap='gray')
            # print(" --- image saved to :", mask_save_path )
        else:
            temp = [int(i) for i in img_name.split(".")[0].split("_")[1].split("x")]
            # w = temp[0]
            # h = temp[1]
            _, h,w = outputs["instances"].to("cpu").pred_masks.numpy().shape
            pred_mask = np.zeros((h,w))
            cv2.imwrite(mask_save_path, pred_mask.astype(int)*255)
            # plt.imsave(mask_save_path, pred_mask.astype(int), cmap='gray')
            print("mask don't exist! image saved to :", mask_save_path )
    ...
    print("masks saved to :",output_mask_save_folder)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--score_type', default='4', type=str, help="(1) IoU (2) Acc (3) F_measure \
                (4)save detectron2 output mask")
    parser.add_argument(
        '--output_mask_save_folder', default='/local-scratch/jiaqit/exp/result/mirror_seg_result/detectron_result', \
             type=str, help="folder to save the output mask")
    parser.add_argument(
    ...
    # ---- calculate score to evaluate the model ----
        ...
        for threshold in range(1,10,1):
            ...
            elif args.score_type == "4":
                print("save output mask from detecton2!")
                save_output_masks(args, predictor)
            # sys.stdout.flush()
