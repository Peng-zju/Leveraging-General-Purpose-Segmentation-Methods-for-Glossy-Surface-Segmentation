...
class SODLoader(Dataset):
    ...
    def __init__(self, mode='train', augment_data=False, target_size=256):
        if mode == 'train':
            self.inp_path = './data/MSD/train/image'
            self.out_path = './data/MSD/train/mask'
        elif mode == 'test':
            self.inp_path = './data/MSD/test/image'
            self.out_path = './data/MSD/test/mask'
        else:
...
class InfDataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #img file name
        img_name = self.img_paths[idx].split('\\')[1].split('.')[0]

        #size
        h, w, c = img.shape

        return img_np, img_tor, img_name, str(h) + ' ' + str(w)
